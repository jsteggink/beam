...
class JarHell {
  ...
  static Set<URL> parseClassPath(String classPath) {
    ...
    for (String element : elements) {
      ...
      // So below is the "correct" code, however in practice with ES, this is usually just
      // a misconfiguration,
      ...
      if (element.isEmpty()) {
        throw new IllegalStateException("Classpath should not contain empty elements! "
            + "(outdated shell script from a previous"
            + " version?) classpath='" + classPath + "'");
      }
      // we should be able to just Paths.get() each element, but unfortunately this is not the
      // whole story on how classpath parsing works: if you want to know,
      // start at sun.misc.Launcher,
        ...
        Files.walkFileTree(root, new SimpleFileVisitor<Path>() {
          ...
          public FileVisitResult visitFile(Path file, BasicFileAttributes attrs)
            ...
            if (entry.endsWith(".class")) {
              ...
              entry = entry.replace(sep, ".").substring(0,  entry.length()
                  - ".class".length());
              checkClass(clazzes, entry, path);
            }
